...
export default function PhotoUploadAnalyzer() {
  ...
  const exportJson = () => {
    if (!result) return;
    const blob = new Blob([JSON.stringify({ notes, result }, null, 2)], {
  ...
  const statusBadge = useMemo(() => {
    ...
    if (score >= 66)
      return (
        <Badge className="bg-green-100 text-green-800">High vegetation</Badge>
      );
    if (score >= 33)
      return (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Capture or select a field photo. We estimate vegetation using
          green-channel index on-device.
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="md:col-span-2 space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Accepts JPEG/PNG from camera or gallery
              </p>
            </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="Add field context (crop, stage, anomalies)"
                  />
                </div>
                <div className="flex gap-2">
                  ...
                  <div className="space-y-2 p-3 border rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold">
                        {result.vegetationScore}/100
                      </span>
                    </div>
                    <Progress value={result.vegetationScore} />
                    <div className="flex items-center justify-between text-sm text-gray-600">
                      <span>
                        {result.width}×{result.height}px · {result.fileSizeKB}{" "}
                        KB
                      </span>
                      {statusBadge}
                    </div>
                    {result.location && (
                      <p className="text-xs text-gray-600 flex items-center">
                        <MapPin className="h-3 w-3 mr-1" /> Lat{" "}
                        {result.location.lat.toFixed(5)}, Lon{" "}
                        {result.location.lon.toFixed(5)}
                      </p>
                    )}
                    <p className="text-xs text-gray-500">
